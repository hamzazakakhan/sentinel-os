...
# Mark docker stack as ready so sentinel-stack.service auto-starts at boot
mkdir -p "${DOCKER_DIR}"
touch "${DOCKER_DIR}/.ready"
ok "Marked docker stack ready (.ready flag)"

# Copy seed SQL into the image so it runs on first DB init
if [[ -f "${PROJECT_DIR}/scripts/seed/seed-postgresql.sql" ]]; then
  mkdir -p "${OVERLAY_DIR}/docker-entrypoint-initdb.d"
  cp "${PROJECT_DIR}/scripts/seed/seed-postgresql.sql" "${OVERLAY_DIR}/docker-entrypoint-initdb.d/01-sentinel-seed.sql"
  ok "Copied Postgres seed SQL for first-boot initialization"
fi
...
[Unit]
Description=Sentinel OS Stack (Docker Compose - all microservices including C4ISR)
...
cat > "${OVERLAY_DIR}/etc/systemd/system/ollama.service" << 'EOF'
[Unit]
Description=Ollama LLM runtime (used by AI service, mission-planner, data-seeder)
...
ExecStartPre=/bin/mkdir -p /var/lib/ollama/models
...
# ── Ollama model bootstrap (runs once on first boot) ──
cat > "${OVERLAY_DIR}/etc/systemd/system/ollama-bootstrap.service" << 'EOF'
[Unit]
Description=Ollama first-boot model pull (tinyllama for tactical inference)
...
Type=oneshot
RemainAfterExit=yes
ExecStartPre=/bin/sh -c 'until curl -fsS http://localhost:11434/api/tags >/dev/null 2>&1; do sleep 3; done'
ExecStart=/bin/sh -c '/usr/local/bin/ollama pull tinyllama || true'
ExecStartPost=/bin/touch /var/lib/ollama/.bootstrap-complete
TimeoutStartSec=900
...
# ── Sentinel data-seeder activator (sets seed mode after stack is ready) ──
cat > "${OVERLAY_DIR}/etc/systemd/system/sentinel-seeder-activate.service" << 'EOF'
[Unit]
Description=Activate Sentinel OS data-seeder service (Ollama-driven continuous seeding)
After=sentinel-stack.service ollama-bootstrap.service
Wants=sentinel-stack.service ollama-bootstrap.service

[Service]
Type=oneshot
ExecStartPre=/bin/sh -c 'for i in $(seq 1 60); do curl -fsS http://localhost:8099/health && break; sleep 5; done'
ExecStart=/bin/sh -c 'curl -fsS -X POST -H "Content-Type: application/json" -d "{\"mode\":\"quiet\"}" http://localhost:8099/mode || true'
RemainAfterExit=yes
...
# Enable all sentinel services on boot
...
ln -sf /etc/systemd/system/ollama.service                  "${OVERLAY_DIR}/etc/systemd/system/multi-user.target.wants/ollama.service"
ln -sf /etc/systemd/system/ollama-bootstrap.service        "${OVERLAY_DIR}/etc/systemd/system/multi-user.target.wants/ollama-bootstrap.service"
ln -sf /etc/systemd/system/sentinel-seeder-activate.service "${OVERLAY_DIR}/etc/systemd/system/multi-user.target.wants/sentinel-seeder-activate.service"
